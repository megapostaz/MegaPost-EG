...
-- 5. إعداد التخزين (Storage) والسياسات
...
-- ✅ التعديل الوحيد هنا (بدون حذف أي كود)
IF item.asin NOT LIKE 'MULTI_%' THEN
...
-- تعديل: السماح للمنشورات المجمعة MULTI بالمرور حتى لو السعر null
IF (item.price IS NULL OR item.price = 0) AND item.asin NOT LIKE 'MULTI_%' THEN 
    CONTINUE; 
END IF;
...
final_price := FLOOR(COALESCE(item.price, 0) * (1 - (COALESCE(item.extra_payment_discount, 0)::float / 100)));
...
price_display := '<b>' || final_price::text || ' ج</b>';
IF old_price_val > final_price THEN
price_display := price_display || ' (بدلاً من <s>' || old_price_val::text || ' ج</s>)';
END IF;
...
IF item.footer_text IS NOT NULL AND trim(item.footer_text) <> '' AND item.footer_text NOT LIKE 'Channels:%' THEN
...
CASE COALESCE(item.template_id, 0)
    WHEN 0 THEN
        IF item.asin LIKE 'MULTI_%' THEN
            final_text := item.affiliate_link;
        ELSE
            final_text := v_clean_title || chr(10) || chr(10) || 
                '<blockquote>🟢 السعر النهائي ⇚ <b>' || final_price || ' جنيه</b>' || 
                CASE WHEN old_price_val > final_price THEN ' (بدلاً من <s>' || old_price_val || ' جنيه</s>)' ELSE '' END ||
                CASE 
                    ...
                    ELSE '' 
                END || '</blockquote>' || chr(10) || chr(10) || '🔗 لينك المنتج:' || chr(10) || item.affiliate_link;
        END IF;

    WHEN 1 THEN
        final_text := v_clean_title || ' بسعر <b>' || final_price || ' ج</b>';
        IF old_price_val > final_price THEN 
            final_text := final_text || ' (بدلاً من <s>' || old_price_val || ' ج</s>)'; 
        END IF;
        final_text := final_text || chr(10) || chr(10) || 'لينك العرض : ' || item.affiliate_link;

    WHEN 2 THEN
        final_text := '💎 ' || v_clean_title || chr(10) || chr(10) || '🏷️ السعر الآن: <b>' || final_price || ' جنيه فقط</b>' || chr(10) || chr(10) || 'لينك العرض :' || chr(10) || item.affiliate_link;

    ELSE
        final_text := v_clean_title || chr(10) || chr(10) || '🔗 لينك المنتج:' || chr(10) || item.affiliate_link;
END CASE;
END IF;
...
-- 8. إعداد الجدولة الزمنية (Cron Jobs)
DO $$ 
BEGIN
...
CREATE POLICY "Anon Banners Full Access" ON storage.objects FOR ALL TO anon USING (bucket_id = 'banners') WITH CHECK (bucket_id = 'banners');
